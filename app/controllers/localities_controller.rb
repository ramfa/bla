class LocalitiesController < ApplicationController
  before_action :set_locality, only: [:show, :edit, :update, :destroy, :for_province_id]

  # GET /localities
  # GET /localities.json
  def index
    if params[:order] == 'name' || params[:order].blank?
      @localities = Locality.order('name').all
    elsif params[:order] == 'province_name'
      @localities = Locality.includes(:province).order('provinces.name').all
    end
  ...
  private
    ...
    def for_province_id
      @locality = Locality.where( params[:id]).sort_by{ |k| k['name'] }
      respond_to do |format|
